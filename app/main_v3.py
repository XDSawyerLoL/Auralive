...
async function load(){s.textContent='Lecture…';const r=await fetch('/api/models/constellation');const j=await r.json();s.textContent=j.last_error?j.last_error:'Prête';rt.textContent=JSON.stringify({route:j.last_route||{},ensemble:j.last_ensemble||{}},null,2);g.innerHTML='';
