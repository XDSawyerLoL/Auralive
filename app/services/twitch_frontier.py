...
FRONTIER_BOT_SCOPES = sorted(
    set(twitch_base.BOT_SCOPES)
    | {
...
FRONTIER_BROADCASTER_SCOPES = sorted(
    set(twitch_base.BROADCASTER_SCOPES)
    | {
        "channel:read:ads",
...
# La route OAuth historique lit ces constantes au niveau du module. Frontier les
# étend avant que le serveur reçoive une demande de connexion Twitch.
twitch_base.BOT_SCOPES = FRONTIER_BOT_SCOPES
twitch_base.BROADCASTER_SCOPES = FRONTIER_BROADCASTER_SCOPES


class FrontierTwitchClient(TwitchClient):
    """Couverture Twitch native étendue, sans service bot intermédiaire."""
    ...
    async def _subscribe_for_role(self, session_id: str, role: str) -> None:
        ...
        if role == "bot":
            subscriptions = [
                ("channel.chat.message", "1", {"broadcaster_user_id": broadcaster, "user_id": bot}),
                ("channel.chat.notification", "1", {"broadcaster_user_id": broadcaster, "user_id": bot}),
                ("channel.chat.clear", "1", {"broadcaster_user_id": broadcaster, "user_id": bot}),
                ("channel.chat.clear_user_messages", "1", {"broadcaster_user_id": broadcaster, "user_id": bot}),
                ("channel.chat.message_delete", "1", {"broadcaster_user_id": broadcaster, "user_id": bot}),
                ("channel.chat_settings.update", "1", {"broadcaster_user_id": broadcaster, "user_id": bot}),
                ("channel.suspicious_user.message", "1", {"broadcaster_user_id": broadcaster, "moderator_user_id": bot}),
                ("channel.suspicious_user.update", "1", {"broadcaster_user_id": broadcaster, "moderator_user_id": bot}),
                ("channel.warning.send", "1", {"broadcaster_user_id": broadcaster, "moderator_user_id": bot}),
                ("channel.warning.acknowledge", "1", {"broadcaster_user_id": broadcaster, "moderator_user_id": bot}),
                ("channel.shield_mode.begin", "1", {"broadcaster_user_id": broadcaster, "moderator_user_id": bot}),
                ("channel.shield_mode.end", "1", {"broadcaster_user_id": broadcaster, "moderator_user_id": bot}),
            ]
            ...
            subscriptions = [
                ...
                ("channel.shoutout.create", "1", {"broadcaster_user_id": broadcaster, "moderator_user_id": broadcaster}),
                ("channel.shoutout.receive", "1", {"broadcaster_user_id": broadcaster, "moderator_user_id": broadcaster}),
    ...
    async def send_announcement(self, message: str, color: str = "primary") -> dict[str, Any]:
        ...
        return await self.request(
            ...
            "/chat/announcements",
            role="bot",
            params={"broadcaster_id": self.broadcaster_user_id, "moderator_id": self.bot_user_id},
    ...
    async def create_stream_marker(self, description: str = "Aura Live") -> dict[str, Any]:
        ...
        payload = await self.request(
            ...
            "/streams/markers",
            role="broadcaster",
            json_body={"user_id": self.broadcaster_user_id, "description": description[:140]},
        )
        rows = payload.get("data", [])
